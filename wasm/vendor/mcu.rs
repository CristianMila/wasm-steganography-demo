...
use crate::components::{SampleRatios, ComponentID};
...
impl<T: ZByteReaderTrait> JpegDecoder<T> {
    ...
    fn decode_mcu_width<const PROGRESSIVE: bool>(
        ...
        let mut secret_chroma_value: BTreeMap<ComponentID, u8> = BTreeMap::new();
                ...
                for v_samp in 0..component.vertical_sample {
                    for h_samp in 0..component.horizontal_sample {
                        ...
                        if component.needed {
                            ...
                            // now here we will retrieve only bits from cb and cr, in that order,
                            // from the last 8 bytes of the 64 element array
                            if component.component_id == ComponentID::Cb || component.component_id == ComponentID::Cr {
                                println!("cb or cr: {:?}", tmp);
                                 // secret info is only encoded in chromation planes
                                let mut byte: u8 = 0;
                                for bit_index in 0..8 {
                                    let coeff_index = 56 + bit_index;
                                    let coeff_value = tmp[coeff_index];
                                    let lsb = (coeff_value & 1) as u8;
                                    byte |= lsb << (7 - bit_index);
                                }
                                
                                secret_chroma_value.insert(component.component_id, byte);
                            }
                        }
                    }
                }
        ...
        // add the secret bytes in the correct order, that is, first cb, then cr
        secret.push(*secret_chroma_value.get(&ComponentID::Cb).unwrap_or(&0));
        secret.push(*secret_chroma_value.get(&ComponentID::Cr).unwrap_or(&0));
        ...
        match &mut self.secret_bytes {
            Some(s) => s.extend_from_slice(&secret),
            None => {
