...
impl<W: JfifWrite> Encoder<W> {
    ...
    /// Set secret to embed into the image
    ...
    fn encode_image_interleaved<I: ImageBuffer, OP: Operations>(
        ...
        let secret = self.secret.clone().unwrap_or(Vec::new());
        let mut secret_bit_reader = {
            ...
            BitReader::endian(Cursor::new(payload), LittleEndian)
        };
                ...
                for (i, component) in self.components.iter().enumerate() {
                    for v_offset in 0..component.vertical_sampling_factor as usize {
                        for h_offset in 0..component.horizontal_sampling_factor as usize {
                            ...
                            OP::only_quantize_block(
                                &block,
                                &mut q_block,
                                &q_tables[component.quantization_table as usize],
                            );

                            // introduce here the secret bits
                            // only use luminance component if luma, chromatic else, but not black
                            if !secret.is_empty() && (self.components.len() == 1 || (self.components.len() >= 3 && (component.id == 1 || component.id == 2))) {
                                for i in 56..64 {
                                    if let Ok(bit) = secret_bit_reader.read_bit() {
                                        if bit {
                                            if q_block[i] % 2 == 0 {
                                                q_block[i] += 1;
                                            }
    ...
    fn encode_blocks<I: ImageBuffer, OP: Operations>(
        ...
        let should_embed_secret = !secret.is_empty();
        ...
        for (i, component) in self.components.iter().enumerate() {
            ...
            for block_y in 0..rows {
                for block_x in 0..cols {
                    ...
                    // only use luminance component if luma, chromatic else, but not black
                    if should_embed_secret
                        && (self.components.len() == 1 || (self.components.len() >= 3 && (component.id == 1 || component.id == 2))) {
                        for i in 56..64 {
                            if let Ok(bit) = secret_bit_reader.read_bit() {
                                if bit {
                                    if q_block[i] % 2 == 0 {
...
pub(crate) trait Operations {
    ...
    #[inline(always)]
    fn zigzag_block(block: &[i16; 64], z_block: &mut [i16; 64]) {
        for i in 0..64 {
    ...
    #[inline(always)]
    fn only_quantize_block(block: &[i16; 64], q_block: &mut [i16; 64], table: &QuantizationTable) {
        for i in 0..64 {
            // let z = ZIGZAG[i] as usize & 0x3f;
            q_block[i] = table.quantize(block[i], i);
        }
    }
