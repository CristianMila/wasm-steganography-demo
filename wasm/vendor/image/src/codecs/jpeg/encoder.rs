...
impl<W: Write> JpegEncoder<W> {
    ...
    pub fn encode_with_secret(
        ...
        match color_type {
            ExtendedColorType::L8 => {
                let image: ImageBuffer<Luma<_>, _> =
                    ImageBuffer::from_raw(width, height, image).unwrap();
                self.encode_image_with_secret(&image, secret)
            }
            ExtendedColorType::Rgb8 => {
                let image: ImageBuffer<Rgb<_>, _> =
                    ImageBuffer::from_raw(width, height, image).unwrap();
    ...
    pub fn encode_image_with_secret<I: GenericImageView>(&mut self, image: &I, secret: String) -> ImageResult<()>
    where
        I::Pixel: PixelWithColorType,
    {
        let n = I::Pixel::CHANNEL_COUNT;
        ...
        if ExtendedColorType::Rgb8 == color_type || ExtendedColorType::Rgba8 == color_type {
            self.encode_rgb_with_secret(image, secret.as_bytes())
        } else {
            panic!("Steganography only supported for RGB images");
            // self.encode_gray(image)
        }?;
    ...
    fn encode_rgb_with_secret<I: GenericImageView>(&mut self, image: &I, secret: &[u8]) -> io::Result<()> {
        let mut y_dcprev = 0;
        let mut cb_dcprev = 0;
        let mut cr_dcprev = 0;
        ...
        for y in (0..image.height()).step_by(8) {
            for x in (0..image.width()).step_by(8) {
                ...
                // add here the hidden bits:
                for i in 56..64 {
                    // hide next bit in the cb plane
                    if let Ok(bit) = BitRead::read_bit(&mut reader) {
                        if bit {
                            if dct_cb_block[i] % 2 == 0 {
                                dct_cb_block[i] += 1;
                            }
                        } else {
                            if dct_cb_block[i] % 2 != 0 {
